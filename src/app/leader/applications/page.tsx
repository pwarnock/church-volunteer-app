...
export default function ApplicationsPage() {
  ...
  useEffect(() => {
    ...
    if (session.user.role !== 'MINISTRY_LEADER' && session.user.role !== 'ADMIN') {
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{application.opportunity.ministry}</p>
                    </div>
                  ...
                  <div className="border-t pt-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <span className="ml-2 text-gray-600">{application.volunteer.name}</span>
                      </div>
                      <div>
                        <span className="font-medium text-gray-700">Email:</span>
                        <span className="ml-2 text-gray-600">{application.volunteer.email}</span>
                      </div>
                    ...
                    {application.volunteer.profile && (
                      <div className="mt-4 space-y-2">
                        {application.volunteer.profile.spiritualGifts && (
                          <div>
                            <span className="font-medium text-gray-700">Spiritual Gifts:</span>
                            <div className="mt-1 flex flex-wrap gap-1">
                              {(() => {
                                try {
                                  const gifts = typeof application.volunteer.profile.spiritualGifts === 'string' 
                                    ? JSON.parse(application.volunteer.profile.spiritualGifts)
                                    : application.volunteer.profile.spiritualGifts;
                                  return Array.isArray(gifts) ? gifts.map((gift, index) => (
                        ...
                        {application.volunteer.profile.interests && (
                          <div>
                            ...
                            <div className="mt-1 flex flex-wrap gap-1">
                              {(() => {
                                try {
                                  const interests = typeof application.volunteer.profile.interests === 'string' 
                                    ? JSON.parse(application.volunteer.profile.interests)
                                    : application.volunteer.profile.interests;
                        ...
                        {application.volunteer.profile.availability && (
                          <div>
                            ...
                            <span className="ml-2 text-gray-600">{application.volunteer.profile.availability}</span>
                          </div>
                        )}
                      </div>
                    )}
