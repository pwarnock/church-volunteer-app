...
describe('Security', () => {
  describe('Password Security', () => {
    ...
    it('should allow strong passwords', () => {
      const strongPasswords = ['SecurePass123', 'MyPassword456', 'ComplexP@ssw0rd']
      const isStrongPassword = (pwd: string) => {
        const hasUpperCase = /[A-Z]/.test(pwd)
        const hasLowerCase = /[a-z]/.test(pwd)
        const hasNumbers = /[0-9]/.test(pwd)
  ...
  describe('Authorization & Access Control', () => {
    ...
    it('should validate user owns resource before access', () => {
      const canAccessResource = (userId: string, resourceOwnerId: string, isAdmin: boolean) => {
  ...
  describe('Data Protection', () => {
    ...
    it('should not store sensitive data in cookies', () => {
      const isSafeCookie = (name: string) => {
        ...
        return !unsafeCookies.some((unsafe) => name.toLowerCase().includes(unsafe))
      }
  ...
  describe('SQL Injection Prevention', () => {
    it('should use parameterized queries (via ORM)', () => {
      ...
      const isSafeQuery = (query: string) => {
        ...
        return !query.includes("'") || query.includes('?') || query.includes('$1')
      }
      ...
      const dangerousQuery = "SELECT * FROM users WHERE email = '" + 'test@example.com' + "'"
  ...
  describe('CORS & Headers Security', () => {
    ...
    it('should validate Content-Type', () => {
      const isValidContentType = (contentType: string) => {
        const valid = ['application/json', 'text/html', 'application/x-www-form-urlencoded']
