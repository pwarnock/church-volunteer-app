 ...
 */
import { LogEntry, ErrorCategory, ErrorSeverity, ErrorMetrics } from './types.js';
...
export class ErrorTracker {
  ...
  getMetrics(): ErrorMetrics {
    ...
    const topErrors = Array.from(this.errorCounts.entries())
      .map(([key, count]) => {
        ...
        const lastOccurred = Math.max(...relatedErrors.map(e => new Date(e.timestamp).getTime()));
        
        return { message: `${category}: ${message}`, count, lastOccurred: new Date(lastOccurred).toISOString() };
      })
  ...
  private cleanup(): void {
    ...
    this.errors = this.errors.filter(error => 
      now - new Date(error.timestamp).getTime() < this.retentionMs
    );
    ...
    const activeKeys = new Set(this.errors.map(e => `${e.category}:${e.message}`));
...
export function categorizeError(error: Error): { category: ErrorCategory; severity: ErrorSeverity } {
  const message = error.message.toLowerCase();
  const stack = error.stack?.toLowerCase() || '';
  
  // Database errors
  if (message.includes('database') || message.includes('sql') || 
      message.includes('connection') || stack.includes('prisma')) {
    return {
  ...
  if (message.includes('security') || message.includes('suspicious') ||
      message.includes('attack') || message.includes('malicious')) {
    return {
