...
const signInAsVolunteer = async (page: any) => {
...
const signInAsLeader = async (page: any) => {
  await page.goto('/auth/signin')
...
test.describe('Opportunities', () => {
  test('volunteer can view opportunities list', async ({ page }) => {
    ...
    const oppLink = page.locator('a:has-text("Opportunities"), button:has-text("Opportunities")')
    ...
    const opportunitiesList = page.locator('[data-testid="opportunities-list"], ul, div:has-text("Opportunity")')
  ...
  test('volunteer can view opportunity details', async ({ page }) => {
    ...
    const firstOpportunity = page.locator('a, button').filter({ hasText: /Teacher|Mentor|Volunteer/ }).first()
    if (await firstOpportunity.isVisible()) {
      await firstOpportunity.click()
      await page.waitForTimeout(1000)

      // Check if details are shown
      const description = page.locator('text=/description|requirements|ministry/i')
  ...
  test('volunteer can apply for opportunity', async ({ page }) => {
    ...
    const applyButton = page.locator('button:has-text("Apply"), button:has-text("Apply Now")')
    if (await applyButton.first().isVisible()) {
      ...
      if (await submitButton.isVisible()) {
        ...
        const successMessage = page.locator('text=/success|submitted|applied/i')
  ...
  test('volunteer can view their applications', async ({ page }) => {
    ...
    const applicationsLink = page.locator('a:has-text("Applications"), button:has-text("My Applications")')
    if (await applicationsLink.isVisible()) {
      ...
      const applicationsList = page.locator('text=/application|status|opportunity/i')
  ...
  test('ministry leader can view opportunity management', async ({ page }) => {
    ...
    const leaderLink = page.locator('a:has-text("Leader"), a:has-text("Manage"), button:has-text("Opportunities")')
  ...
  test('ministry leader can create opportunity', async ({ page }) => {
    ...
    const createButton = page.locator('button:has-text("Create"), a:has-text("New"), button:has-text("Add Opportunity")')
    ...
    const titleInput = page.locator('input[placeholder*="title"], input[placeholder*="opportunity"]').first()
    if (await titleInput.isVisible()) {
      await titleInput.fill('Test Opportunity')

      // Fill description if available
      const descInput = page.locator('textarea[placeholder*="description"], input[placeholder*="description"]').first()
      if (await descInput.isVisible()) {
        await descInput.fill('This is a test opportunity')
      }

      // Submit form
      const submitButton = page.locator('button:has-text("Create"), button:has-text("Submit"), button:has-text("Save")')
      if (await submitButton.isVisible()) {
        await submitButton.click()
  ...
  test('ministry leader can view applications', async ({ page }) => {
    ...
    const applicationsLink = page.locator('a:has-text("Applications"), button:has-text("View Applications")')
    if (await applicationsLink.isVisible()) {
      await applicationsLink.click()
      await page.waitForTimeout(1000)

      // Check if applications are displayed
      const applicationsList = page.locator('text=/volunteer|application|status/i')
      await expect(applicationsList).toBeVisible({ timeout: 5000 })
    } else {
